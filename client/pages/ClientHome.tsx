...
import { Bell, Flame, Activity, Droplet, ArrowRight, Users, Apple, MessageCircle, Sparkles, Award, Zap } from 'lucide-react';
...
export default function ClientHome() {
  ...
  const quickTiles = [
    { icon: Users, label: 'Trainers', path: '/discover', color: 'from-blue-400 to-blue-600' },
    { icon: Apple, label: 'Nutritionists', path: '/discover', color: 'from-green-400 to-green-600' },
    { icon: Droplet, label: 'Meal Tracker', path: '/meals', color: 'from-orange-400 to-orange-600' },
    { icon: MessageCircle, label: 'CoCircle', path: '/community', color: 'from-pink-400 to-pink-600' },
    { icon: Sparkles, label: 'Quests', path: '/quests', color: 'from-purple-400 to-purple-600' },
    { icon: Award, label: 'Become Trainer', path: '/trainer-signup', color: 'from-yellow-400 to-yellow-600' },
  ];
      ...
      <div className="sticky top-0 z-40 backdrop-blur-2xl bg-white/80 border-b border-gray-200/50 px-4 py-4">
        <div className="max-w-md mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">Hi, {userProfile?.full_name?.split(' ')[0] || 'Friend'}</h2>
            </div>
          </div>
          <button
      ...
      <div className="max-w-md mx-auto px-4 py-6 space-y-6">
        ...
        <div className="relative h-40 rounded-3xl overflow-hidden backdrop-blur-xl bg-white/50 border border-white/20 shadow-lg">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-gray-900/50 to-transparent flex flex-col justify-end p-4">
            <p className="text-white font-bold text-lg">Focus: {userProfile?.primary_focus || 'General Fitness'}</p>
            <p className="text-gray-200 text-sm">Today's goal: Complete your daily activities</p>
          </div>
        </div>
        ...
        <div className="backdrop-blur-2xl bg-white/90 border border-white/20 rounded-3xl shadow-xl p-6 space-y-6">
            ...
            <div className="relative w-32 h-32 rounded-full bg-gradient-to-r from-orange-400 to-yellow-400 flex items-center justify-center shadow-lg">
              <div className="absolute inset-1 rounded-full bg-white flex items-center justify-center">
                <div className="text-center">
                  <p className="text-3xl font-bold text-gray-900">{Math.round(stepsPercent)}%</p>
                  <p className="text-xs text-gray-600">{stats.steps.toLocaleString()} / {stats.stepsGoal.toLocaleString()}</p>
                </div>
              </div>
              <div
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="backdrop-blur-xl bg-white/50 rounded-2xl p-3 border border-gray-200/50 text-center">
              <p className="text-2xl font-bold text-orange-500">{stats.calories}</p>
              <p className="text-xs text-gray-600">Calories</p>
            </div>
            <div className="backdrop-blur-xl bg-white/50 rounded-2xl p-3 border border-gray-200/50 text-center">
              <p className="text-2xl font-bold text-blue-500">{stats.water}ml</p>
              <p className="text-xs text-gray-600">Water</p>
            </div>
            <div className="backdrop-blur-xl bg-white/50 rounded-2xl p-3 border border-gray-200/50 text-center">
              <p className="text-2xl font-bold text-teal-500">{stats.distance.toFixed(1)}km</p>
          ...
          <div className="space-y-3">
            <div>
              <div className="flex justify-between mb-1">
                ...
                <span className="text-xs text-gray-600">{Math.round(caloriesPercent)}%</span>
              </div>
            ...
            <div>
              <div className="flex justify-between mb-1">
                ...
                <span className="text-xs text-gray-600">{Math.round(waterPercent)}%</span>
              </div>
              <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                <div
        ...
        {bmi && (
          <div className="backdrop-blur-2xl bg-white/90 border border-white/20 rounded-3xl shadow-xl p-6 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="text-center">
                <p className="text-3xl font-bold text-orange-500">{bmi.toFixed(1)}</p>
                <p className="text-xs text-gray-600 mt-1">BMI</p>
              </div>
              <div className="text-center">
                <p className="text-xl font-bold text-gray-700">{bmiStatus || 'Normal'}</p>
                <p className="text-xs text-gray-600 mt-1">Status</p>
              </div>
              <div className="text-center">
                <div className="text-center">
                  <p className="text-sm font-semibold text-gray-700">{userProfile?.height_cm || '-'} cm</p>
                  <p className="text-sm font-semibold text-gray-700">{userProfile?.weight_kg || '-'} kg</p>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            {quickTiles.map((tile, idx) => (
              ...
              >
                <tile.icon size={24} />
                <p className="text-xs font-semibold text-center">{tile.label}</p>
              </button>
            ))}
          </div>
        </div>
            ...
            <div className="flex gap-3">
              {[...Array(3)].map((_, i) => (
                ...
                >
                  ...
                  <p className="text-sm font-medium text-gray-900">Suggestion will appear here</p>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-bold text-gray-900">Latest from CoCircle</h3>
            <button
          ...
          <div className="space-y-3">
            {[...Array(2)].map((_, i) => (
              ...
              >
                ...
                <p className="text-sm text-gray-700">Post caption will appear here</p>
