...
export default function MobileMessages() {
          ...
          {selectedMessages.map((message: any, idx: number) => (
            ...
            >
              ...
              >
                ...
                <p
                  className={`text-xs mt-1 ${message.sender_id === localStorage.getItem("userId") ? "text-orange-100" : theme === "dark" ? "text-gray-400" : "text-gray-500"}`}
                >
  ...
  return (
    <div className={`min-h-screen pb-20 ${theme === "dark" ? "bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950" : "bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50"}`}>
      ...
      {/* New Message Button */}
      <button className="fixed bottom-24 right-4 w-14 h-14 rounded-full bg-gradient-to-r from-orange-500 to-yellow-500 hover:from-orange-600 hover:to-yellow-600 text-white flex items-center justify-center shadow-lg hover:shadow-xl transition-all hover:scale-110 active:scale-95">
        <Plus size={28} />
      </button>
    </div>
  );
